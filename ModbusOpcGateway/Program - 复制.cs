...
#region 第一版本，未使用 FluentModbus 库，直接使用 NModbus4 的 TcpListener 模式，手动处理连接和事件。
//using System;
...
//    //    public bool GetStatusCoil() { lock (_lock) return _status; }
...
//    //    public void SetStatus(bool s) { lock (_lock) _status = s; }
//    //    public void SetMode(SimulationMode m) { lock (_lock) _mode = m; }
//    //    public void SetNoiseMultiplier(float m) { lock (_lock) _noise = m; }
//    //    public void SetResponseDelay(int ms) { lock (_lock) _delayMs = ms; }
//    //}
...
//        //    var tasks = new[]
//        //    {
//        //        RunGenerator(shared, cts.Token),
//        //        RunModbusServer(shared, cts.Token)
//        //    };

//        //    Console.WriteLine("> Modbus TCP on port 5020");
...
//            // 3. 创建取消令牌源 (CancellationTokenSource)
...
//        //static async Task RunGenerator(SharedData data, CancellationToken token)
//        //{
...
//        //        var noise = data.GetNoiseMultiplier();
//        //        var delay = data.GetResponseDelayMs();
...
//        //        float t = (float)(rand.NextDouble() * 10.0 + 20.0);
//        //        float p = (float)(rand.NextDouble() * 20.0 + 90.0);

//        //        if (mode == SharedData.SimulationMode.Trend)
//        //        {
//        //            var seconds = DateTime.UtcNow.Second + DateTime.UtcNow.Minute * 60;
//        //            t += (float)Math.Sin(seconds / 60.0 * Math.PI * 2) * 2.0f;
//        //            p += (float)Math.Sin(seconds / 30.0 * Math.PI * 2) * 3.0f;
//        //        }

//        //        t += (float)((rand.NextDouble() - 0.5) * 2.0 * noise);
//        //        p += (float)((rand.NextDouble() - 0.5) * 2.0 * noise);
//        //        bool s = rand.Next(2) == 1;
...
//                // 3. 生成基础随机数据
//                // 模拟传感器的基础读数范围
//                float t = (float)(rand.NextDouble() * 10.0 + 20.0); // 温度：20.0 ~ 30.0 °C
//                float p = (float)(rand.NextDouble() * 20.0 + 90.0); // 压力：90.0 ~ 110.0 kPa
...
//        //static async Task RunModbusServer(SharedData data, CancellationToken token)
//        //{
...
//        //    // 创建一次从站，复用
//        //    var slave = ModbusTcpSlave.CreateTcp(1, listener);
//        //    //var factory = new ModbusFactory();
//        //    //var slave = factory.CreateSlave(1);  // 单位 ID = 1
...
//        //                        // 同步 Listen 方法—— NModbus4 阻塞直到连接断开
//        //                        await slave.ListenAsync();
//        //                    }
//        //                    catch (Exception ex) 
//        //                    { 
//        //                        Console.WriteLine($"[Modbus] Client error: {ex.Message}"); 
//        //                    }
//        //                }
//        //            }, token);
//        //        }
//        //        catch (OperationCanceledException) { break; }
//        //        catch (Exception ex) { Console.WriteLine($"[Modbus] Accept error: {ex.Message}"); }
//        //    }
...
//        /// <summary>
//        /// 模拟标准的 Modbus TCP 从站（Slave/Server），
//        /// 允许外部的 PLC、SCADA 系统或调试工具通过网络来读取或写入数据。
//        /// </summary>
//        //static async Task RunModbusServer(SharedData data, CancellationToken token)
//        //{
//        //    var endpoint = new IPEndPoint(IPAddress.Any, 5020);
...
//        //                    if (addr == 0) e.Data[i] = data.GetTempReg();
//        //                    else if (addr == 1) e.Data[i] = data.GetPressReg();
//        //                    else if (addr == 10) e.Data[i] = (ushort)data.GetMode();
//        //                    else if (addr == 11) e.Data[i] = (ushort)(data.GetNoiseMultiplier() * 100);
//        //                    else if (addr == 12) e.Data[i] = (ushort)data.GetResponseDelayMs();
//        //                }
//        //            }
...
//        //                {
//        //                    ushort addr = (ushort)(e.StartAddress + i);
//        //                    if (addr == 0) e.Data[i] = data.GetStatusCoil() ? (ushort)1 : (ushort)0;
//        //                }
//        //            }
//        //        };
...
//        //                    ushort addr = (ushort)(e.StartAddress + i);
//        //                    ushort val = e.Data[i];
//        //                    if (addr == 10) data.SetMode((SharedData.SimulationMode)val);
//        //                    else if (addr == 11) data.SetNoiseMultiplier(val / 100f);
//        //                    else if (addr == 12) data.SetResponseDelay(val);
//        //                }
//        //            }
//        //            else if (e.ModbusDataType == ModbusDataType.Coil)
//        //            {
...
//        //                {
//        //                    ushort addr = (ushort)(e.StartAddress + i);
//        //                    if (addr == 0) data.SetStatus(e.Data[i] != 0);
//        //                }
//        //            }
//        //        };
...
//        //            catch (Exception ex)
//        //            {
//        //                if (!(ex is OperationCanceledException) && !(ex is ObjectDisposedException) && !(ex is System.Net.Sockets.SocketException))
...
//        //        // --- 读取事件 (DataStoreReadFrom) ---
//        //        // 触发时机：主站发起读取请求时。
//        //        // 任务：在库从 DataStore 抓取数据发送之前，先更新 DataStore 内部的最新值。
//        //        dataStore.DataStoreReadFrom += (obj, e) =>
//        //        {
//        //            // 将 obj 转换为具体的 DataStore 类型，以便访问内部数组
//        //            if (obj is not DataStore store) return;
...
//        //                    else
//        //                    {
//        //                        hasValue = false; // 未定义的地址，不更新
//        //                    }

//        //                    // 【关键步骤 3】：将计算结果写入 DataStore 的内部数组
//        //                    // 库随后会从这个数组读取数据发送给主站
//        //                    if (hasValue)
//        //                    {
//        //                        store.HoldingRegisters[addr] = valueToWrite;
...
//        //        // --- 写入事件 (DataStoreWrittenTo) ---
//        //        // 触发时机：主站发起写入请求，且库已经更新了内部数组后。
//        //        // 任务：同步更新我们的业务对象 (SharedData)。
//        //        dataStore.DataStoreWrittenTo += (obj, e) =>
//        //        {
//        //            if (e.ModbusDataType == ModbusDataType.HoldingRegister)
//        //            {
...
#region 代码片段：完整的模拟器实现（包含配置映射、共享数据中心、数据生成器和 Modbus 服务器）
//using System;
//using System.IO;
//using System.Collections.Concurrent;
//using System.Net;
...
//                var tasks = new[]
//                {
//                    RunGenerator(shared, cts.Token, appSettings.Simulation), // 传入仿真配置
...
//        /// 并实时更新到 SharedData 中，供 Modbus 服务器读取。
//        /// </summary>
//        static async Task RunGenerator(SharedData data, CancellationToken token, SimulationSettings config)
//        {
