...
本项目采用**模块化、可独立部署**的架构设计，各项目拥有**独立的核心库**，通过 **MQTT** 进行数据解耦：
...
│  ┌──────────────────────┐   │   ┌──────────────────────┐                │
│  │ ModbusOpcGateway.Core│   │   │ ModbusClientService  │                │
│  │ ├ GeneratorService   │   │   │ OpcUaClientService   │                │
│  │ ├ ModbusServerService│   │   └──────────┬───────────┘                │
│  │ ├ OpcUaServerService │   │              │                            │
│  │ └ SharedData         │   │              ▼                            │
│  └──────────┬───────────┘   │   ┌──────────────────────┐                │
...
| 核心库 | 命名空间 | 引用方 | 说明 |
|--------|----------|--------|------|
| **ModbusOpcGateway.Core** | `ModbusOpcGateway.Core` | ModbusOpcGateway、单元测试 | 模拟器专用核心库 |
| **Industrial.Core** | `Industrial.Core` | BlazorScadaHmi | HMI 专用核心库 |

两个核心库代码独立，各项目可自由演进，互不影响。
...
ModbusOpcGateway.Core/            # 模拟器核心类库（ModbusOpcGateway 专用）
├── Services/
...
├── MqttSubscriberService.cs      # MQTT Subscriber 服务
├── RegisterMap.cs                # 寄存器定义
└── AppSettingsMap.cs             # 配置类

Industrial.Core/                  # HMI 核心类库（BlazorScadaHmi 专用）
├── Services/                     # （结构同 ModbusOpcGateway.Core）
├── Models/
├── SharedData.cs
├── ScadaHub.cs
└── ...                           # 与 ModbusOpcGateway.Core 独立演进
...
- **核心库拆分**：将 Industrial.Core 复制为 ModbusOpcGateway.Core，ModbusOpcGateway 与 BlazorScadaHmi 各自拥有独立核心库，可自由演进互不影响
- **命名空间独立**：ModbusOpcGateway.Core 使用 `ModbusOpcGateway.Core` 命名空间，Industrial.Core 保持 `Industrial.Core` 不变
- **单元测试迁移**：ModbusOpcGateway_xUnit 改为引用 ModbusOpcGateway.Core
- **新增 Gateway_Memory.md**：项目变更记录文件，用于追踪重要架构决策
